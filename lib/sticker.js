...
const video2webp = async (inputBuffer, fps = 10, sec) => {
	let duration = sec ? (sec > 10 ? 9 : sec) : 9
	const tempInputFile = path.join(tmpdir(), `${crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.mp4`)
	const outputFile = path.join(tmpdir(), `${crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`)
	fs.writeFileSync(tempInputFile, inputBuffer);
	while (duration > 0) {
		await new Promise((resolve, reject) => {
			fluent_ffmpeg(tempInputFile)
				.outputOptions([
					'-vcodec libwebp',
					'-vf',
					`scale=320:320:force_original_aspect_ratio=decrease,fps=${fps}, pad=320:320:-1:-1:color=0x00000000, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`,
					'-t ' + duration,
					'-loop 0',
					'-fs 1000000',
					'-vsync 2'
