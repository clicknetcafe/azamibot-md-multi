...
import { fileTypeFromBuffer, fileTypeFromStream, fileTypeStream } from 'file-type'
	...
	let sock = Object.defineProperties(conn, {
		...
		getFile: {
			async value(PATH, saveToFile = false) {
				...
				else if (/^https?:\/\//.test(PATH)) data = await got(PATH).buffer()
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
				...
				try {
					...
					console.error(e)
					return await conn.sendMsg(jid, { ...message, [mtype]: buffer }, { ...opt, ...options })
						.catch(e => (error = e))
				} finally {
					file.clear()
