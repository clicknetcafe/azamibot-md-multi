...
`.trim()
		...
		sendButton: {
			...
			async value(jid, text = '', footer = '', buffer, buttons, quoted, copy, urls, options = {}) {
				...
				// sendButton & sendList inspired from https://github.com/Guru322/GURU-BOT/blob/main/lib/simple.js
				else if (buffer) {
					try {
						file = await conn.getFile(buffer)
						// if not android, send normal message instead of button
						if (await getDevice(quoted?.id) !== 'android') {
							let mtype = '',
								buffer = file.data,
								mimetype = options.mimetype || file.mime,
								convert
							...
							if (/image/.test(mimetype) || (/webp/.test(mimetype) && options.asImage)) mtype = 'image'
							else if (/video/.test(mimetype)) mtype = 'video'
							else mtype = 'document'
							if (options.asDocument) mtype = 'document'
							if (Buffer.byteLength(buffer) > 70000000) mtype = 'document'
							...
							let message = {
								...
								[mtype]: buffer,
								mimetype,
								fileName: file.filename || `file.${file.ext}`
							}
							try {
								return await conn.sendMsg(jid, message, { ...opt, ...options })
							} catch (e) {
								return await conn.reply(jid, e.message, quoted)
							}
		...
		text: {
			get() {
				...
				const text = (typeof msg === 'string' ? msg : msg?.text) || msg?.caption
					|| msg?.contentText || msg?.nativeFlowResponseMessage || ''
