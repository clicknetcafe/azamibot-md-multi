...
const ephemeral = { ephemeralExpiration: WA_DEFAULT_EPHEMERAL }
	...
	let sock = Object.defineProperties(conn, {
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
				...
				let error = false
				try {
					return await conn.sendMessage(jid, message, { ...opt, ...options, ...ephemeral })
				} catch (e) {
					console.error(e)
					return await conn.sendMessage(jid, { ...message, [mtype]: await toBuffer() }, { ...opt, ...options, ...ephemeral })
						.catch(e => (error = e))
				} finally {
					file.clear()
...
`.trim()
		...
		copyNForward: {
			...
			async value(jid, message, forwardingScore = true, options = {}) {
				...
				await conn.relayMessage(jid, m.message, { messageId: m.key.id, additionalAttributes: { ...options, ...ephemeral } })
				return m
			},
			enumerable: true,
			writable: true,
		...
		fakeReply: {
			...
			value(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid, options = {}) {
				return conn.reply(jid, text, { key: { fromMe: areJidsSameUser(fakeJid, conn.user.id), participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options, ...ephemeral})
			},
			writable: true,
		},
		downloadM: {
		...
		sendGroupV4Invite: {
			...
			async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail, options = {}) {
				...
				await conn.relayMessage(participant, message.message, { messageId: message.key.id, additionalAttributes: { ...options, ...ephemeral } })
				return message
			},
			enumerable: true,
			writable: true,
