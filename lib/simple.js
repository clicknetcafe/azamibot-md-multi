	...
	let sock = Object.defineProperties(conn, {
		...
		getFile: {
			...
			async value(PATH, saveToFile = false) {
				...
				if (!streamWithType.fileType) throw new TypeError('Result is not a buffer, please input a valid path, buffer, or direct link buffer')
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
				...
				if (!file || file.ext == 'xml') throw new TypeError('Result is not a buffer')
				let mtype = '',
					stream = file.data,
				...
				let error = false
				try {
					return await conn.sendMessage(jid, message, { ...opt, ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL })
				} catch (e) {
					console.error(e)
					return await conn.sendMessage(jid, { ...message, [mtype]: await toBuffer() }, { ...opt, ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL })
						.catch(e => (error = e))
				} finally {
					file.clear()
...
`.trim()
				...
				}, { quoted, ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL })
			},
			enumerable: true,
			writable: true,
		},
		reply: {
			...
			value(jid, text = '', quoted, options = {}) {
				return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text }, { quoted, ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL })
			},
			writable: true,
		},
		preSudo: {
		...
		sendButton: {
			...
			async value(jid, text = '', footer = '', buffer, buttons, quoted, options = {}) {
				...
				if (!options) options = {}
				if (options.asLocation) buff = await resize(await file.toBuffer(), 300, 150)
				let message = {
					...options,
		...
		cMod: {
			...
			value(jid, message, text = '', sender = conn.user.jid, options = {}) {
				...
				if (typeof content !== 'string') {
					msg[mtype] = { ...content, ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL }
		...
		copyNForward: {
			...
			async value(jid, message, forwardingScore = true, options = {}) {
				...
				await conn.relayMessage(jid, m.message, { messageId: m.key.id, additionalAttributes: { ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL } })
				return m
			},
			enumerable: true,
			writable: true,
		...
		fakeReply: {
			...
			value(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid, options = {}) {
				return conn.reply(jid, text, { key: { fromMe: areJidsSameUser(fakeJid, conn.user.id), participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL})
			},
			writable: true,
		},
		downloadM: {
		...
		sendGroupV4Invite: {
			...
			async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail, options = {}) {
				...
				await conn.relayMessage(participant, message.message, { messageId: message.key.id, additionalAttributes: { ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL } })
				return message
			},
			enumerable: true,
			writable: true,
				...
				return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
					...
					forward: {
						...
						value(jid, force = false, options = {}) {
							...
							}, { ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL })
						},
						enumerable: true,
					},
					copyNForward: {
