...
const ephemeral = { ephemeralExpiration: 86400 }
				...
				let message = {
					...
					...(buffer ?
						options.asLocation && /image/.test(file.mime) ? {
							location: {
								...options,
								jpegThumbnail: await resize(await file.toBuffer(), 300, 169)
							}
						} : {
...
export function serialize() {
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		quoted: {
			get() {
				...
				return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
					...
					device: {
						get() {
							const sender = getDevice((contextInfo.participant || this.chat || '').decodeJid())
		...
		device: {
			get() {
				const sender = getDevice(this.sender)
				return sender ? sender : null
			},
			enumerable: true
		},
