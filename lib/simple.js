	...
	let sock = Object.defineProperties(conn, {
		...
		getFile: {
			...
			async value(PATH, saveToFile = false) {
				...
				else if (/^https?:\/\//.test(PATH)) data = await got.stream(PATH)
		...
		sendMsg: {
			async value(jid, message = {}, options = {}) {
				return await conn.sendMessage(jid, message, { ...options, ephemeralExpiration: 86400 })
			},
			enumerable: true,
			writable: true
		},
		...
		sendFAudio: {
			async value(jid, audioinfo = {}, m, title, thumbnailUrl, sourceUrl, body = '', LargerThumbnail = true, AdAttribution = true) {
				return await conn.sendMsg(m.chat, {
					...audioinfo,
					contextInfo: {
						externalAdReply: {
							title: title,
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
				...
				try {
					...
					console.error(e)
					return await conn.sendMsg(jid, { ...message, [mtype]: await toBuffer() }, { ...opt, ...options })
						.catch(e => (error = e))
				} finally {
					file.clear()
...
export function serialize() {
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		quoted: {
			get() {
				...
				return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
					...
					device: {
						get() {
							const device = getDevice((contextInfo.participant || this.chat || '').decodeJid())
		...
		device: {
			get() {
				const device = getDevice(this.sender)
				return device ? device : null
			},
			enumerable: true
		},
