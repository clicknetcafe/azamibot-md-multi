...
export default async function (m, conn = { user: {} }) {
	...
	let sender = (parsePhoneNumber('+'+m.sender?.replace('@s.whatsapp.net', ''))?.number?.international || '') + (_name ? ' ~' + _name : '')
	...
	let me = parsePhoneNumber('+'+(conn.user?.jid || conn.user?.id)?.replace('@s.whatsapp.net', '') || '')?.number?.international || ''
	console.log(`
...
`.trim(),
	...
	if (m.messageStubParameters) console.log(m.messageStubParameters.map(jid => {
		...
		return chalk.gray((parsePhoneNumber('+' + jid.replace('@s.whatsapp.net', ''))?.number?.international || '') + (name ? ' ~' + name : ''))
	}).join(', '))
