...
import { somematch } from './others.js'
 ...
 *  isChild?: boolean; 
 *  connectionOptions?: Partial<import('@whiskeysockets/baileys').UserFacingSocketConfig>; 
 *  store: typeof store 
 * }} opts
 */
async function start(oldSocket = null, opts = { store }) {
	...
	let conn = makeWASocket({
		...
		browser: ['ZerenityID', 'Chrome', '4.0.0']
	})
...
async function connectionUpdate(update) {
	...
	if (connection === 'close' && code) {
		// use this if u want auto clear data store and sessions when connection closed
		/*let dirname = path.join(path.dirname(fileURLToPath(import.meta.url)), '../')
		let dstore = dirname+'data.store.json'
		if (fs.existsSync(dirname+'eng.traineddata')) fs.unlinkSync(dirname+'eng.traineddata')
		if (fs.existsSync(dstore) && code != 515) {
			fs.unlinkSync(dstore)
			console.log('- cleaning data store -')
		}
		fs.readdir(dirname+'sessions/', (err, files) => {
			if (err) console.log(err)
			files.forEach(file => {
				const fileDir = path.join(dirname+'sessions/', file)
				if (file !== 'creds.json') fs.unlinkSync(fileDir)
			})
