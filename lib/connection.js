...
import { readdir, statSync, writeFileSync, unlinkSync } from 'fs'
...
async function reload(conn, restartConnection, opts = { store, authState, }) {
	/*const fstore = join(dirname(fileURLToPath(import.meta.url)), '../data.store.json')
	const fsessions = join(dirname(fileURLToPath(import.meta.url)), '../sessions/')
	const file = statSync(fstore)
	console.log(file)
	if (file.size > 1180023) {
		writeFileSync(fstore, JSON.stringify({ "chats": {}, "messages": {} }))
		store.readFromFile(fstore)
		//unlinkSync(fstore)
		readdir(fsessions, (err, files) => {
			if (err) console.log(err)
			files.forEach(file => {
				const fileDir = join(fsessions, file)
				if (file !== 'creds.json') unlinkSync(fileDir)
			})
