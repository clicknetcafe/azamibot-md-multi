...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	try {
		if (somematch(['justina','ryujin','rose','blackpink','kpop'], text)) {
			...
			await conn.sendButton(m.chat, `${command} > ${text}`, `_© ${packname} - ${author}_`, anu, [[`⧑ next ${text} ⧑`, `${usedPrefix + command} ${text}`]], m)
			...
			if (!text || text == 'random') {
				try {
					let fimg = await fetch(`https://api.lolhuman.xyz/api/random/cecan?apikey=${global.api}`)
					let fimgb = Buffer.from(await fimg.arrayBuffer())
					if (Buffer.byteLength(fimgb) < 22000) throw Error('[!] Error : No Buffer.')
					await conn.sendButton(m.chat, `_Random pic: cecan_`, packname + ' - ' + author, fimgb, [[`⧑ next ${command} ⧑`, `${usedPrefix + command} ${text}`]], m)
				} catch (e) {
					console.log(e)
					let res = await fetch(`https://raw.githubusercontent.com/clicknetcafe/Database/main/${pickRandom(['Cecan/cecan','Cecan/cecan2'])}.json`)
					let anu = pickRandom(await res.json())
					await conn.sendButton(m.chat, `${command} > random`, `_© ${packname} - ${author}_`, anu, [[`⧑ next ${text} ⧑`, `${usedPrefix + command} ${text}`]], m)
				}
			} else {
