		...
		if (chat.viewonce && m.message.viewOnceMessage) {
			try {
				...
				if (/video/.test(media.mimetype)) {
					await this.sendMessage(m.chat, { video: buffer, caption: i, mentions: j }, { quoted: gfkontak })
				} else {
					await this.sendMessage(m.chat, { image: buffer, caption: i, mentions: j }, { quoted: gfkontak })
				}
			} catch (e) {
				console.log(e)
			}
