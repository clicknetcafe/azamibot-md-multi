...
let handler = async (m, { conn, usedPrefix, command }) => {
		...
		if (command.includes('toa')) {
			await conn.sendMsg(m.chat, { audio: audio.data, mimetype: 'audio/mp4' }, { quoted: m })
		} else {
			let date = new Date().toISOString().replace('-', '').split('T')[0].replace('-', '')
			let meh = padLead(ranNumb(9999), 4)
			await conn.sendMsg(m.chat, {document: audio.data, mimetype: 'audio/mpeg', fileName: `AUD-${date}-WA${meh}.mp3`}, { quoted : m })
		}
}
